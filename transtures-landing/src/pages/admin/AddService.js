...
const AddService = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(t('service_added_successfully'));
  ...
  return (
    <Container fluid className="p-4">
      <h1 className="mb-4">{t('add_new_service')}</h1>
      <Form onSubmit={handleSubmit}>
        ...
        <div className="d-grid gap-2">
          <Button variant="primary" type="submit" disabled={loading}>
            {loading ? t('adding_service') : t('add_service')}
          </Button>
          <Button variant="secondary" onClick={() => navigate('/admin/servicios')} disabled={loading}>
            {t('cancel')}
          </Button>
        </div>
      </Form>
